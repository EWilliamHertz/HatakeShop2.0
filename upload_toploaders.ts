...
const SELLER_ID = 1; 
const BASE_DIR = "Toploaders_extracted/Toploaders picture";
...
async function run() {
  ...
  for (const folder of folders) {
     const pt = folder.trim(); 
     const title = `${pt.toLowerCase()} Toploaders`; 

     const folderPath = path.join(BASE_DIR, folder);
     const files = fs.readdirSync(folderPath).filter(f => f.match(/\.(jpg|jpeg|png|JPG)$/i));
     ...
     const description = `Premium toploaders for trading cards. Size: ${pt}. Price is to be discussed via RFQ.`;
  ...
  console.log("Done uploading Toploaders!");
  process.exit(0);
}
