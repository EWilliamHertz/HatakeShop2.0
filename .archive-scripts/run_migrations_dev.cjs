...
async function migrate(dbName) {
  const client = new Client({ host: process.env.SQL_HOST, user: process.env.SQL_USER, password: process.env.SQL_PASSWORD, database: dbName });
...
migrate('cloud_sql_development_database').catch(console.error);
